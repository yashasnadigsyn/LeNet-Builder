...
def generate_pytorch_code(nodes, edges):
    ...
    while current_node:
        if current_node.data['content'] == 'Image':
            ...
            layer_count += 1
        elif current_node.data['content'] == 'Sigmoid':
            code += f"        self.sigmoid{layer_count} = nn.Sigmoid()\n"
            layer_count += 1
        elif current_node.data['content'] == 'Tanh':
            code += f"        self.tanh{layer_count} = nn.Tanh()\n"
            layer_count += 1
        elif current_node.data['content'] == 'ReLU':
            code += f"        self.relu{layer_count} = nn.ReLU()\n"
            layer_count += 1
    ...
    while current_node:
        if current_node.data['content'] == 'Image':
            ...
            code += f"        x = self.conv{layer_count}(x)\n"
        elif current_node.data['content'].startswith('Pool'):
            code += f"        x = self.pool{layer_count}(x)\n"
        elif current_node.data['content'] == 'Sigmoid':
            code += f"        x = self.sigmoid{layer_count}(x)\n"
        elif current_node.data['content'] == 'Tanh':
            code += f"        x = self.tanh{layer_count}(x)\n"
        elif current_node.data['content'] == 'ReLU':
            code += f"        x = self.relu{layer_count}(x)\n"
        elif current_node.data['content'] == 'Flatten':
            code += f"        x = self.flatten{layer_count}(x)\n"
        elif current_node.data['content'].startswith('Dense'):
            code += f"        x = self.fc{layer_count}(x)\n"
...
with tab3:
    ...
    This is just a simple version of LeNet builder. I am still learning Deep Learning. This is my first prototype.
    I learnt about LeNet today and was fascinated by it and built this. I am gonna add new features everytime i learn something new about DL.

    Thank You!

    **Connect with me:**
    - [GitHub](https://github.com/yashasnadigsyn)
    """)
